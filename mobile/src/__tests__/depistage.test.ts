...
describe('effectuerDepistage : en ligne', () => {
  it('renvoie le résultat officiel du serveur et enregistre le dépistage comme synchronisé', async () => {
    ...
    const r = await effectuerDepistage('enfant', mesureEnfant, { personneRef: ' E-7 ' });
    ...
    expect(enregistre.extras).toMatchObject({ personne_ref: 'E-7', oedemes_incertains: false, recommandation: 'Transfert immédiat' });
  });
...
describe('paramsResultat', () => {
  it("transmet l'état provisoire, le doublon et les avertissements à l'écran de résultat", () => {
    ...
    expect(p).toMatchObject({ population: 'enfant', provisoire: '1', doublon: '1', historique_ok: '1', recommandation: '' });
  });
});
